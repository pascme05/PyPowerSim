...
#######################################################################################################################
"""
This function calculates the results for a transient solution of the B6 full-bridge circuit.
...
# Function
#######################################################################################################################
def calcTransB6(mdl, para, setup):
    ###################################################################################################################
    # MSG IN
    ###################################################################################################################
    print("------------------------------------------")
    ...
    for i in range(0, len(id1)):
        v_ref[id1[i]] = (Vdc / 2) * Mi * genWave(t_ref, fel, phiV - i * 2 / 3 * np.pi, setup)
        e_ref[id1[i]] = E * genWave(t_ref, fel, phiE - i * 2 / 3 * np.pi, setup)
    ...
    if setup['Par']['PWM']['type'] == "FF":
        [xs, xsh, s, c, x, n0] = calcSSeqB6_FF(v_ref, t_ref, Mi, setup)
    elif setup['Par']['PWM']['type'] == "CB":
        [xs, xsh, s, c, x, n0] = calcSSeqB6_CB(v_ref, t_ref, Mi, setup)
    elif setup['Par']['PWM']['type'] == "SV":
        [xs, xsh, s, c, x, n0] = calcSSeqB6_SV(v_ref, t_ref, Mi, setup)
    elif setup['Par']['PWM']['type'] == "OPP":
        [xs, xsh, s, c, x, n0] = calcSSeqB6_OPP(v_ref, t_ref, Mi, setup)
    else:
        [xs, xsh, s, c, x, n0] = calcSSeqB6_CB(v_ref, t_ref, Mi, setup)
    ...
    # Time Domain
    # ==============================================================================
    [timeAc, timeDc] = calcTimeB6(t_ref, s, e_ref, Vdc, Mi, mdl, setup, Nsim * (K - 1), (K * Nsim + 1))
    ...
    for _ in tqdm(range(iterNel), desc='Elec-Period', position=0):
        ...
        for ii in range(iterNpwm):
            ...
            for j in range(0, len(id2)):
                timeElec['sw'][id2[j]] = calcElecSwi(Vdc, timeAc[id4[j]][start:ende], (s[id3[j]][start:ende] == (-1) ** j), Tj[j], id5[j], para, setup)
