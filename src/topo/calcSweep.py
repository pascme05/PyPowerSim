...
def calcSweep(top, mdl, _, setup):
    ...
    for i in tqdm(range(len(M_i)), desc='Sweep'):
        ...
        # Distortion
        # ------------------------------------------
        [numDistAc, numDistDc] = calcDistNum(t_ref[start:ende], tempAc['i_a'], tempAc['v_a'], tempDc['i_dc'],
                                             tempDc['v_dc'], Vdc, fel)
        [anaTimeAc, anaTimeDc] = top.calcDist(M_i[i], L, Z)
    ...
    # Output
    # ==============================================================================
    [time, freq, sweep] = top.out(timeAc, timeDc, freqSw, freqAc, freqDc, distAc, distDc, t_ref, v_ref, e_ref, s, c,
                                  xs, xsh, x, xN0, M_i, start, ende)
