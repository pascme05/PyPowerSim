...
#######################################################################################################################
"""
This function calculates the results for a transient solution of the B4 full-bridge circuit.
...
# Function
#######################################################################################################################
def calcTransB4(mdl, para, setup):
    ###################################################################################################################
    # MSG IN
    ###################################################################################################################
    print("------------------------------------------")
    ...
    Tj = setup['Dat']['stat']['Tj'] * np.ones((4, 1))
    ...
    # Reference
    # ------------------------------------------
    v_ref['A'] = +(Vdc / 2) * Mi * genWave(t_ref, fel, phiV, setup)
    v_ref['B'] = -(Vdc / 2) * Mi * genWave(t_ref, fel, phiV, setup)
    ...
    if setup['Par']['PWM']['type'] == "FF":
        [xs, xsh, s, c] = calcSSeqB4_FF(v_ref, t_ref, Mi, setup)
    elif setup['Par']['PWM']['type'] == "CB":
        [xs, xsh, s, c] = calcSSeqB4_CB(v_ref, t_ref, Mi, setup)
    elif setup['Par']['PWM']['type'] == "OPP":
        [xs, xsh, s, c] = calcSSeqB4_OPP(v_ref, t_ref, Mi, setup)
    else:
        [xs, xsh, s, c] = calcSSeqB4_CB(v_ref, t_ref, Mi, setup)
    ...
    # Time Domain
    # ==============================================================================
    [timeAc, timeDc] = calcTimeB4(t_ref, s, e_ref, Vdc, Mi, mdl, setup, Nsim*(K-1), (K*Nsim + 1))
    ...
    for _ in tqdm(range(iterNel), desc='Elec-Period', position=0):
        ...
        for ii in range(iterNpwm):
            ...
            for j in range(0, len(id2)):
                timeElec['sw'][id2[j]] = calcElecSwi(Vdc, id4[j] * timeAc['i_a'][start:ende], (s[id3[j]][start:ende] == (-1) ** j), Tj[j], id5[j], para, setup)
                timeLoss['sw'][id2[j]] = calcLossSwi(s[id3[j]][start:ende] * (-1) ** j, timeElec['sw'][id2[j]]['i_T'], timeElec['sw'][id2[j]]['i_D'], timeElec['sw'][id2[j]]['v_T'], timeElec['sw'][id2[j]]['v_D'], Tj[j], para, setup)
    ...
    for i in range(0, len(id2)):
        if setup['Par']['Ther']['Heatsink'] == 1 and setup['Par']['Ther']['Coupling'] == 1:
            ...
            Pv_Base = np.mean((out['loss']['sw']['S1']['p_L'] + out['loss']['sw']['S2']['p_L'] + out['loss']['sw']['S3']['p_L'] + out['loss']['sw']['S4']['p_L'])) / 4
