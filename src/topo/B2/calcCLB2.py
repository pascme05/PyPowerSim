...
from src.topo.B2.calcControllerB2 import calcHysteresisB2
...
# Function
#######################################################################################################################
def calcCLB2(mdl, para, setupTopo, setupData, setupPara, setupExp):
    ###################################################################################################################
    # MSG IN
    ###################################################################################################################
    print("------------------------------------------")
    print("START: Closed-Loop Simulation B2")
    print("------------------------------------------")
    ...
    #[xs, xsh, s, c] = calcHysteresisB2(v_ref, t_ref, dTs, Mi, setupPara, setupTopo)
    #Move this in function:
    initState = 1
    hysteresisTol = np.max(i_ref) * 0.1

    s = np.ones(np.size(t_ref)) * initState
    currState = initState
    for i in range(0, np.size(t_ref)-dTs, dTs):
        [timeAc, timeDc] = calcTimeB2(t_ref, s, e_ref, Vdc, Mi, mdl, setupTopo, i, (i + dTs))
        if timeAc['i_a'][-1] - i_ref[(i + dTs)] > hysteresisTol:
            currState = -1
        elif timeAc['i_a'][-1] - i_ref[(i + dTs)] < -hysteresisTol:
            currState = 1
        s[((i + dTs) +1):np.size(s)] = currState


    # ==============================================================================
    # Time Domain
    # ==============================================================================
    # Move this into calcHysteresisB2
    [timeAc, timeDc] = calcTimeB2(t_ref, s, e_ref, Vdc, Mi, mdl, setupTopo, Nsim * (K - 1), (K * Nsim + 1))

    # Test plots:
    import matplotlib.pyplot as plt
    [timeAc, timeDc] = calcTimeB2(t_ref, s, e_ref, Vdc, Mi, mdl, setupTopo, 0, (K * Nsim + 1))
    plt.plot(i_ref)
    plt.plot(timeAc['i_a'])
    plt.show()
    # ==============================================================================
    ...
    for _ in tqdm(range(iterNel), desc='Elec-Period', position=0):
        ...
        for ii in tqdm(range(iterNpwm), desc='PWM-Period', position=1, leave=False):
            ...
            for j in range(0, len(id2)):
                timeElec['sw'][id2[j]] = calcElecSwi(Vdc, timeAc['i_a'][start:ende], (s[start:ende] == (-1) ** j),
                                                     Tj[j], id5[j], para, setupPara)
                timeLoss['sw'][id2[j]] = calcLossSwi(s[start:ende] * (-1) ** j, timeElec['sw'][id2[j]]['i_T'],
                                                     timeElec['sw'][id2[j]]['i_D'],
                                                     timeElec['sw'][id2[j]]['v_T'], timeElec['sw'][id2[j]]['v_D'],
                                                     Tj[j], para, setupPara, setupExp)

                if setupPara['Ther']['Heatsink'] == 1 & setupPara['Ther']['Coupling'] == 1:
                    [timeTher['sw'][id6[j]], Tinit_T[:, j]] = calcTherRC(Tinit_T[:, j], Ta,
                                                                         timeLoss['sw'][id2[j]]['p_T'],
                                                                         t_ref[start:ende], Rth_JA, Cth_JA)
                    [timeTher['sw'][id8[j]], Tinit_C[:, j]] = calcTherRC(Tinit_C[:, j], Ta,
                                                                         timeLoss['sw'][id2[j]]['p_L'],
            ...
            [timeTher['cap']['C1'], Tinit_Cap] = calcTherRC(Tinit_Cap, Ta, timeLoss['cap']['C1']['p_L'],
                                                            t_ref[start:ende], Rth_JA_cap, Cth_JA_cap)
    ...
    for i in range(0, len(id2)):
        if setupPara['Ther']['Heatsink'] == 1 & setupPara['Ther']['Coupling'] == 1:
            [out['ther']['sw'][id8[i]], _] = calcTherRC(0, Ta, out['loss']['sw'][id2[i]]['p_L'].values, t, Rth_CA,
                                                        Cth_CA)
    ...
    [freqSw, freqAc, freqDc] = calcFreq(s[Nsim:(K * Nsim + 1)], s[Nsim:(K * Nsim + 1)], timeAc, timeDc)
    # ==============================================================================
    # Output
    # ==============================================================================
    c=s                                                                                                                     #!!!!!!!
